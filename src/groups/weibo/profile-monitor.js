...
export function createWeiboProfileMonitor(settings) {
  ...
  const configuredDefaultUrls = uniqueUrls(settings.defaultUrls);
  const defaultUrls = configuredDefaultUrls.length ? configuredDefaultUrls : [""];
  const defaultLimit = asInteger(settings.defaultLimit, DEFAULT_OPTIONS.limit, 1, 100);
  const defaultLimitVersion = asInteger(settings.defaultLimitVersion, 0, 0, 1000);
  ...
  const jsonListKey = settings.jsonListKey || `${targetOptionKey}s`;
  const dataFilterDefinitions = Object.freeze(Array.isArray(settings.dataFilters) ? [...settings.dataFilters] : []);
  const dataColumns = Object.freeze(Array.isArray(settings.dataColumns) ? [...settings.dataColumns] : []);
  const buildExportRows = typeof settings.buildExportRows === "function" ? settings.buildExportRows : (rows) => rows;
  const mergeDataRow = typeof settings.mergeDataRow === "function"
    ? settings.mergeDataRow
    : (previous, row) => ({ ...previous, ...row });
  const exportFilenamePrefix = settings.exportFilenamePrefix || `${settings.platformId || "weibo"}-${featureId}-data`;
  const normalizeDataRow = typeof settings.normalizeDataRow === "function" ? settings.normalizeDataRow : (row) => row;
  const normalizeDataRows = (rows) => (Array.isArray(rows) ? rows : []).map((row) => normalizeDataRow(row));
  const buildJsonParameters = (state) => {
    const parameters = {
      [jsonListKey]: [...state.profileUrls]
    };
    if (hasLimit) parameters.limit = state.limit;
    parameters.concurrency = state.concurrency;
    parameters.intervalMinMs = state.intervalMinMs;
    parameters.intervalMaxMs = state.intervalMaxMs;
    parameters.forceUpdateData = Boolean(state.forceUpdateData);
    if (supportsPolling) {
      parameters.polling = Boolean(state.polling);
      parameters.pollingMinutes = state.pollingMinutes;
    }
    return parameters;
  };
  const normalizeJsonParameters = (input, state) => {
    if (!input || typeof input !== "object" || Array.isArray(input)) {
      throw new Error("JSON 顶层必须是一个对象。");
    }
    if (!Array.isArray(input[jsonListKey])) {
      throw new Error(`${jsonListKey} 必须是字符串数组。`);
    }
    const profileUrls = uniqueUrls(input[jsonListKey]);
    if (!profileUrls.length && !input[jsonListKey].length) {
      throw new Error(`${jsonListKey} 至少需要保留一个输入项。`);
    }
    if (!profileUrls.length) profileUrls.push("");
    const invalidUrl = profileUrls.find((url) => url && !validTargetUrl(url));
    if (invalidUrl) throw new Error(`${subjectLabel}链接格式不正确：${invalidUrl}`);
    if (supportsPolling && input.polling !== undefined && typeof input.polling !== "boolean") {
    ...
    const intervalRange = normalizeProfileIntervalRange({
      intervalMinMs: input.intervalMinMs ?? state.intervalMinMs,
      intervalMaxMs: input.intervalMaxMs ?? state.intervalMaxMs
    });
    return {
      profileUrls,
      limit: hasLimit ? asInteger(input.limit, state.limit, 1, 100) : state.limit,
      ...
      polling: supportsPolling ? (input.polling ?? state.polling) : false,
      pollingMinutes: supportsPolling
        ? asInteger(input.pollingMinutes, state.pollingMinutes, 1, 1440)
        : state.pollingMinutes
    };
  };
  ...
  const renderRunButton = (state, className = "") => state.mode === "runner"
    ? renderFeatureRunnerModeButton(state.runnerPanel, { className, primaryClass: "xhs-primary-button", stopClass: "xhs-stop-button", disabled: state.running })
    : state.running
  ...
  const renderOptions = (state) => {
    ...
    const forceUpdateControl = `<label class="xhs-switch-control"><input type="checkbox" data-field="forceUpdateData" ${state.forceUpdateData ? "checked" : ""} ${disabled}><span><strong>强制更新数据</strong><small>遇到相同数据时，使用本次采集结果覆盖本地旧数据。</small></span></label>`;
    const pollingControls = `<div class="xhs-polling-row">${forceUpdateControl}${supportsPolling ? `<label class="xhs-switch-control"><input type="checkbox" data-field="polling" ${state.polling ? "checked" : ""} ${disabled}><span><strong>循环监控</strong><small>每轮任务完成后按设定周期再次执行，直到手动停止。</small></span></label><label class="xhs-control compact ${state.polling ? "" : "is-disabled"}"><span>轮询周期</span><div class="xhs-input-with-unit"><input type="number" min="1" max="1440" value="${state.pollingMinutes}" data-field="pollingMinutes" ${state.polling && !state.running ? "" : "disabled"}><span>分钟</span></div></label>` : ""}</div>`;
  ...
  const renderData = (state) => {
    ...
    return `<section class="xhs-content-card xhs-table-page"><div class="xhs-panel-head"><div><h2>数据</h2><p>共 ${state.dataRows.length} 条，最多保留 ${formatLimitValue(state.dataStorageLimit)}；${settings.dataSummary}</p></div>${renderDataColumnSettingsPanel({ columns: dataColumns, visibility: state.dataColumnVisibility, expanded: state.dataColumnsOpen, escapeHtml })}</div>${renderDataFilterPanel({ rows: state.dataRows, definitions: dataFilterDefinitions, values: state.dataFilters, expanded: state.dataFiltersOpen, escapeHtml })}<div class="xhs-table-shell data" tabindex="0"><table class="xhs-table xhs-data-table">${renderConfiguredDataTable({ rows: exportRows, columns: dataColumns, visibility: state.dataColumnVisibility, escapeHtml, emptyText, actionColumn: state.boundRunnerActions?.tableColumn(exportRows, escapeHtml, filteredRows) })}</table></div></section>`;
  };
  ...
  return async function mountWeiboProfileMonitor(container, context) {
    ...
    const runnerPanel = getFeatureRunnerPanelState(featureKey);
    const state = {
      tab: "params", mode: runnerPanel.running ? "runner" : "form", runnerPanel, jsonDraft: "", profileUrls: Array.isArray(saved?.profileUrls) && saved.profileUrls.length ? saved.profileUrls : [...defaultUrls], inputListPage: 1, dataStorageLimit: storageLimits.dataStorageLimit, taskRecordsPerStatusLimit: storageLimits.taskRecordsPerStatusLimit, records: limitProfileRecordsPerStatus(saved?.records, storageLimits.taskRecordsPerStatusLimit), dataRows: applyItemLimit(normalizeDataRows(saved?.dataRows), storageLimits.dataStorageLimit), batchOpen: false, batchDraft: "", guideOpen: false, taskDetailRecordId: "", limit: Number(savedConfig.defaultLimitVersion || 0) < defaultLimitVersion ? defaultLimit : asInteger(savedConfig.limit, defaultLimit, 1, 100), concurrency: normalizeTaskConcurrency(savedConfig.concurrency), intervalMinMs: savedRange.intervalMinMs, intervalMaxMs: savedRange.intervalMaxMs, forceUpdateData: normalizeForceUpdateData(savedConfig.forceUpdateData), polling: supportsPolling && Boolean(savedConfig.polling), pollingMinutes: asInteger(savedConfig.pollingMinutes, DEFAULT_OPTIONS.pollingMinutes, 1, 1440), optionsOpen: false, pageParametersOpen: false, running: Boolean(activeRuns.get(runScope) && !activeRuns.get(runScope).stopRequested), stopping: false, recordFilters: { profile: ALL_RECORD_FILTER, status: ALL_RECORD_FILTER, executionType: ALL_RECORD_FILTER }, dataFilters: createDataFilterValues(dataFilterDefinitions), dataFiltersOpen: true, dataColumnVisibility: createDataColumnVisibility(dataColumns, saved?.dataColumnVisibility), dataColumnsOpen: false, notice: saved?.notice || { tone: "info", text: settings.initialNotice }
    };
    saveState(state);
    let disposed = false;
    let activeRun = activeRuns.get(runScope) || null;
    const syncJsonDraft = () => { state.jsonDraft = JSON.stringify(buildJsonParameters(state), null, 2); };
    const applyJsonDraft = () => {
      try {
        Object.assign(state, normalizeJsonParameters(JSON.parse(state.jsonDraft), state));
    ...
    const finishRecord = (id, startedAtMs, statusKey, resultCount = 0, addedCount = 0, error = "") => {
      const meta = STATUS[statusKey] || STATUS.error;
      state.records = updateProfileRecord(state.records, id, { status: meta.label, statusKey, tone: meta.tone, resultCount: Number(resultCount) || 0, addedCount: Number(addedCount) || 0, error, duration: formatDuration(Date.now() - startedAtMs) }, state.taskRecordsPerStatusLimit);
    };
    ...
    const mergeRows = (data, task, profileUrl) => {
      const rows = normalizeDataRows(settings.toRows(data, profileUrl)).map((row) => withCanonicalRecord(featureKey, {
        ...row,
        rawPageText: row.rawPageText || data?.rawPageText || data?.detail?.rawPageText || data?.profile?.rawPageText || ""
      }));
      const merged = mergeDataRowsByKey({
        ...
        mergeRow: (previous, row) => tagTaskDataRow(mergeDataRow(previous, row), task)
      });
      state.dataRows = applyItemLimit(merged.rows, state.dataStorageLimit);
      return { added: merged.addedCount, count: rows.length };
    };
    const startRun = async () => {
      ...
      try {
        do {
          ...
          await runConcurrentTasks(profileUrls, {
            ...
            worker: async (profileUrl, index) => {
              ...
              try {
                ...
                finishRecord(current.record.id, current.startedAtMs, "success", result.count, result.added);
              } catch (error) {
                if (control.stopRequested) finishRecord(current.record.id, current.startedAtMs, "stopped");
                else {
                  failed += 1;
                  const message = error.message || String(error);
                  finishRecord(current.record.id, current.startedAtMs, "error", 0, 0, message);
          ...
          if (!supportsPolling || !state.polling) { state.notice = { tone: failed ? "warning" : "success", text: `运行结束：新增 ${addedTotal} 条${rowLabel}数据，失败 ${failed} 个${subjectLabel}。` }; state.tab = addedTotal ? "data" : "records"; break; }
    ...
    const handleClick = async (event) => {
      ...
      if (await handleFeatureRunnerPanelAction(state.runnerPanel, action, {
        ...
        getParameters: () => buildJsonParameters(state),
        onChange: render,
        onFinished: async () => {
          const latest = await loadSavedState().catch(() => null);
          state.records = limitProfileRecordsPerStatus(latest?.records || state.records, state.taskRecordsPerStatusLimit);
          state.dataRows = Array.isArray(latest?.dataRows) ? applyItemLimit(normalizeDataRows(latest.dataRows), state.dataStorageLimit) : state.dataRows;
      ...
      switch (action) {
        ...
        case "set-mode": {
          ...
          if (nextMode === "runner") syncFeatureRunnerDraft(state.runnerPanel, buildJsonParameters(state), { force: true });
          state.mode = nextMode;
          render();
          break;
        }
        ...
        case "show-all-data-columns": state.dataColumnVisibility = showAllDataColumns(dataColumns); saveState(state); scheduleDataColumnRender(render); break;
        ...
        case "copy-json": {
          ...
          try {
            openRowsJsonPreview(rows, (currentRows) => projectDataRowsByColumns(
              buildExportRows(currentRows), dataColumns, state.dataColumnVisibility
            ));
          } catch (error) {
        ...
        case "export-csv": {
          const rows = filterDataRows(state.dataRows, dataFilterDefinitions, state.dataFilters);
          downloadDataRowsCsv(buildExportRows(rows), dataColumns, state.dataColumnVisibility, exportFilenamePrefix);
          state.notice = { tone: "success", text: `已导出 ${rows.length} 条筛选后的表格数据（CSV）。` };
          render();
          break;
        }
        case "reset": Object.assign(state, { profileUrls: [...defaultUrls], inputListPage: 1, limit: defaultLimit, concurrency: DEFAULT_OPTIONS.concurrency, intervalMinMs: DEFAULT_OPTIONS.intervalMinMs, intervalMaxMs: DEFAULT_OPTIONS.intervalMaxMs, forceUpdateData: false, polling: false, pollingMinutes: DEFAULT_OPTIONS.pollingMinutes, optionsOpen: false, pageParametersOpen: false, notice: { tone: "success", text: "已还原主页输入与基础运行选项。" } }); syncJsonDraft(); saveState(state); render(); break;
        default: break;
      }
    };
    ...
    const handleChange = (event) => {
      ...
      if (dataColumn) {
        ...
        state.dataColumnVisibility = createDataColumnVisibility(dataColumns, state.dataColumnVisibility);
        saveState(state);
      ...
      if (["limit", "concurrency", "intervalMinMs", "intervalMaxMs", "forceUpdateData", ...(supportsPolling ? ["polling", "pollingMinutes"] : [])].includes(field)) { saveState(state); render(); }
    };
    const handleKeydown = (event) => { if (event.key === "Escape" && (state.batchOpen || state.guideOpen || state.taskDetailRecordId)) closeModal(); };
    syncFeatureRunnerDraft(state.runnerPanel, buildJsonParameters(state));

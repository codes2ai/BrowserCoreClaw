...
- **统一数据投影**：每条新采集结果保留功能原始字段，同时写入 `canonical` 统一记录；内容归为 `content`（新闻、笔记、博文、视频），博主资料归为 `profile`，便于数据传输、远程归档与后续跨平台分析。
...
- 每条新采集数据均保存功能原始字段与 `canonical` 标准字段。`canonical.platformExtra.rawPageText` 保存采集当刻页面完整可见文本，`platformExtra.rawFields` 保存结构化原始字段快照；数据传输页默认按统一字段读取，功能页与既有导出继续使用原始字段。
...
| 实体 | 覆盖范围 | 关键字段 |
| --- | --- | --- |
| `content` | Google 新闻、小红书笔记、微博博文、抖音作品 | `contentType`、作者、正文/摘要、发布时间、媒体、互动指标、来源上下文 |
| `profile` | 小红书、微博、抖音的博主公开资料 | `profile`（昵称、头像、简介、标签等）及关注/粉丝/获赞等指标 |

Google 新闻以 `content/news` 写入。无法横向抽象的平台特有内容统一放入 `platformExtra`，不会丢失；每条记录还含 `sourceContext`（关键词、来源主页、页面顺序等）。完整字段定义、中文说明和 JSON 示例见 [doc/data-schema.md](doc/data-schema.md)。
...
│   │       └── styles.css                # 功能私有样式
│   └── shared/                           # 跨功能复用能力，包括 canonical-data 统一数据投影
├── doc/data-schema.md                    # 统一传输字段、中文说明与 JSON 示例
